    ...
    try {
      ...
      if (!test1) {
        ...
        const shoppingCart = await User.findOneAndUpdate(
          { username },
          { $push: { shoppingCart: newISBN } }
        );
        res
          .status(200)
          .json({ message: `Book ${newISBN} added to shopping cart` });
      }
    } catch (error) {
      res.status(404).json({ message: error });
    }
  ...
  static deleteBook = async (req, res) => {
    ...
    try {
      ...
      if (!test1) {
        ...
        const shoppingCart = await User.findOneAndUpdate(
          { username },
          { $pull: { shoppingCart: newISBN } }
        );

        res
          .status(200)
          .json({ message: `Book ${newISBN} removed from shopping cart` });
      }
    } catch (error) {
      res.status(404).json({ message: error });
    }
